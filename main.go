...
func main() {
	...
	rows, err := db.Query("select name,url,thumburl from stuff")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
	for rows.Next() {
		...
		var url string
		var thumburl string
		err = rows.Scan(&name, &url, &thumburl)
		if err != nil {
			log.Fatal(err)
		}
		arrayOfItems = append(arrayOfItems, Item{Name: name, url: url, ThumbURL: thumburl})
	...
	b.Handle(tb.OnQuery, func(q *tb.Query) {
		...
		for i, item := range arrayOfItems {
			if strings.Contains(item.Name, "photo") {
